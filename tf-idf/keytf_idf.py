...
def befwry(str):  # 写入预处理，将list转为string
    outall = ''
    for i in str:
...
def main():
    swpath = r'D:\爬虫数据表格\stop_word.txt'  # 停用词表路径
    ...
    for i in filelist:
        ...
        echout = str(tfidfdic[0][0]) + ',' + str(tfidfdic[1][0]) + ',' + str(tfidfdic[2][0]) + ',' + str(tfidfdic[3][0]) + ',' + str(tfidfdic[4][0])
        print(title + ' is ok!')
        #outall = outall + echout
        f = codecs.open(r'D:\爬虫数据\\' + title, 'ab', 'ANSI')
        f.write(echout)
        f.close()
        #wrypath = r"D:\text\ll"+str(i+1)+".txt"
        #print(wry(echout, wrypath) + ' is ok!')
